 ...
 * $Id: app.php 33828 2008-02-22 09:25:26Z team $
 */
...
class App
{
	...
	public function IsUsed(){
		global $tqb;
		return $tqb->CheckPlugin($this->id);
	}
	...
	public function GetManageUrl(){
		global $tqb;
		return $tqb->host . 'content/' . $this->type . '/' . $this->id . '/' . $this->path;
	}

	public function GetDir(){
		global $tqb;
		return $tqb->path . 'content/' . $this->type . '/' . $this->id . '/';
	}

	public function GetLogo(){
		global $tqb;
		if($this->type=='plugin'){
			return $tqb->host . 'content/' . $this->type . '/' . $this->id . '/logo.png';
		}else{
			return $tqb->host . 'content/' . $this->type . '/' . $this->id . '/screenshot.png';
		}
	}
	...
	public function GetCssFiles(){
		global $tqb;
		$dir = $tqb->contentdir . 'theme/' . $this->id . '/style/';
		return GetFilesInDir($dir,'css');
	}

	public function LoadInfoByXml($type,$id){
		global $tqb;
		$path=$tqb->contentdir . $type . '/' . $id . '/' . $type . '.xml';
	...
	public function SaveInfoByXml(){
		...
		$path=$tqb->contentdir . $this->type . '/' . $this->id . '/' . $this->type . '.xml';
