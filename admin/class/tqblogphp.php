...
class TQBlogPHP {
    ...
    public $captchaurl = null;
    private $isgzip = false;

    static public function GetInstance() {
        if (!isset(self::$_tqb)) {
            self::$_tqb = new TQBlogPHP;
        }
        return self::$_tqb;
    }

    function __construct() {
        global $option, $lang, $blogpath, $bloghost, $cookiespath, $contentdir, $table, $datainfo;
        global $blogversion, $blogtitle, $blogname, $blogsubname, $blogtheme, $blogstyle;

        TQBlogException::SetErrorHook();
        ...
        if (trim($this->option['CFG_BLOG_CLSID']) == '') {
            $this->option['CFG_BLOG_CLSID'] = GetGuid();
        }
        $this->guid = $this->option['CFG_BLOG_CLSID'];
        ...
        $this->managecount = $this->option['CFG_MANAGE_COUNT'];
        $this->pagebarcount = $this->option['CFG_PAGEBAR_COUNT'];
        $this->searchcount = $this->option['CFG_SEARCH_COUNT'];
        $this->displaycount = $this->option['CFG_DISPLAY_COUNT'];
        $this->commentdisplaycount = $this->option['CFG_COMMENTS_DISPLAY_COUNT'];
    ...
    function __call($method, $args) {
        foreach ($GLOBALS['Filter_Plugin_Tqb_Call'] as $fpname => &$fpsignal) {
            $fpreturn = $fpname($method, $args);
            if ($fpsignal == PLUGIN_EXITSIGNAL_RETURN) {
                return $fpreturn;
            }
        }
        if ($this->option['CFG_DEBUG_MODE'] == true)
            $this->ShowError(81, __FILE__, __LINE__);
    }

    function __set($name, $value) {
        foreach ($GLOBALS['Filter_Plugin_Tqb_Set'] as $fpname => &$fpsignal) {
    ...
    function __get($name) {
        foreach ($GLOBALS['Filter_Plugin_Tqb_Get'] as $fpname => &$fpsignal) {
...
//********************************************************
    ...
    public function Initialize() {
        ...
        if ($this->option['CFG_SITE_TURNOFF'] == true) {
            Http503();
        ...
        if ($this->option['CFG_PERMANENT_DOMAIN_ENABLE'] == true) {
            $this->host = $this->option['CFG_BLOG_HOST'];
            $this->cookiespath = substr($this->host, strpos($this->host, '/', 8));
        } else {
            $this->option['CFG_BLOG_HOST'] = $this->host;
        }

        $this->option['CFG_BLOG_VERSION'] = CFG_BLOG_VERSION;
        $this->option['CFG_BLOG_PRODUCT_FULL'] = $this->option['CFG_BLOG_PRODUCT'] . ' ' . $this->option['CFG_BLOG_VERSION'];
        $this->option['CFG_BLOG_PRODUCT_FULLHTML'] = '<a href="http://www.tqtqtq.com/" title="TQBlog" target="_blank">' . $this->option['CFG_BLOG_PRODUCT_FULL'] . '</a>';

        date_default_timezone_set($this->option['CFG_TIME_ZONE_NAME']);
        ...
        header('Product:' . $this->option['CFG_BLOG_PRODUCT_FULL']);

        $this->captchaurl = $this->host . 'admin/script/captcha.php';
    ...
    public function LoadManage() {
        ...
        if ($this->user->Status == CFG_MEMBER_STATUS_AUDITING)
            $this->ShowError(79, __FILE__, __LINE__);
        if ($this->user->Status == CFG_MEMBER_STATUS_LOCKED)
            $this->ShowError(80, __FILE__, __LINE__);

        $this->CheckTemplate();

        foreach ($GLOBALS['Filter_Plugin_Tqb_LoadManage'] as $fpname => &$fpsignal)
            $fpname();
    }
    ...
    public function Terminate() {
        foreach ($GLOBALS['Filter_Plugin_Tqb_Terminate'] as $fpname => &$fpsignal)
            $fpname();
    ...
    public function OpenConnect() {
        if ($this->isconnect)
            return false;
        if (!$this->option['CFG_DATABASE_TYPE'])
            return false;
        switch ($this->option['CFG_DATABASE_TYPE']) {
            case 'sqlite':
            case 'sqlite3':
                try {
                    $this->InitializeDB($this->option['CFG_DATABASE_TYPE']);
                    if ($this->db->Open(array(
                                $this->contentdir . 'data/' . $this->option['CFG_SQLITE_NAME'],
                                $this->option['CFG_SQLITE_PRE']
                            )) == false) {
            ...
            default:
                try {
                    if ($this->InitializeDB($this->option['CFG_DATABASE_TYPE']))
                        return false;
                    if ($this->db->Open(array(
                                $this->option['CFG_MYSQL_SERVER'],
                                $this->option['CFG_MYSQL_USERNAME'],
                                $this->option['CFG_MYSQL_PASSWORD'],
                                $this->option['CFG_MYSQL_NAME'],
                                $this->option['CFG_MYSQL_PRE'],
                                $this->option['CFG_MYSQL_PORT'],
                                $this->option['CFG_MYSQL_PERSISTENT']
                            )) == false) {
...
#Cache相关
    ...
    public function SaveCache() {
        ...
        #$s=$this->contentdir . 'cache/' . $this->guid . '.cache';
...
#保存tqb设置函数
    ...
    public function SaveOption() {
        ...
        $this->option['CFG_BLOG_CLSID'] = $this->guid;

        if (strpos('|SAE|BAE2|ACE|', '|' . $this->option['CFG_YUN_SITE'] . '|') === false) {
            $s = "<?php\r\n";
            $s.="return ";
            $s.=var_export($this->option, true);
            $s.="\r\n?>";
            @file_put_contents($this->path . 'config.php', $s);
        }
    ...
    public function LoadOption() {
        ...
        foreach ($array as $key => $value) {
            //if($key=='CFG_PERMANENT_DOMAIN_ENABLE')continue;
            //if($key=='CFG_BLOG_HOST')continue;
            //if($key=='CFG_BLOG_CLSID')continue;
            if ($key == 'CFG_YUN_SITE')
                continue;
            if ($key == 'CFG_BLOG_LANGUAGEPACK')
                continue;
            if ($key == 'CFG_DATABASE_TYPE')
                continue;
            if ($key == 'CFG_SQLITE_NAME')
                continue;
            if ($key == 'CFG_SQLITE_PRE')
                continue;
            if ($key == 'CFG_MYSQL_SERVER')
                continue;
            if ($key == 'CFG_MYSQL_USERNAME')
                continue;
            if ($key == 'CFG_MYSQL_PASSWORD')
                continue;
            if ($key == 'CFG_MYSQL_NAME')
                continue;
            if ($key == 'CFG_MYSQL_CHARSET')
                continue;
            if ($key == 'CFG_MYSQL_PRE')
                continue;
            if ($key == 'CFG_MYSQL_ENGINE')
                continue;
            if ($key == 'CFG_MYSQL_PORT')
                continue;
            if ($key == 'CFG_MYSQL_PERSISTENT')
                continue;
...
#权限及验证类
    ...
    function CheckRightsByLevel($level, $action) {
        ...
        foreach ($GLOBALS['Filter_Plugin_Tqb_CheckRightsByLevel'] as $fpname => &$fpsignal) {
...
#
    function BuildModule() {
        ...
        foreach ($GLOBALS['Filter_Plugin_Tqb_BuildModule'] as $fpname => &$fpsignal)
            $fpname();
...
#加载函数
    ...
    public function LoadThemes() {
        $dirs = GetDirsInDir($this->contentdir . 'theme/');
    ...
    public function LoadPlugins() {
        $dirs = GetDirsInDir($this->contentdir . 'plugin/');
...
#模板相关函数
    ...
    public function MakeTemplatetags() {
        ...
        $option = $this->option;
        unset($option['CFG_BLOG_CLSID']);
        unset($option['CFG_SQLITE_NAME']);
        unset($option['CFG_SQLITE3_NAME']);
        unset($option['CFG_MYSQL_USERNAME']);
        unset($option['CFG_MYSQL_PASSWORD']);
        unset($option['CFG_MYSQL_NAME']);

        $this->templatetags['tqb'] = &$this;
        ...
        $this->templatetags['email_admin'] = $this->option['CFG_BLOG_EMAIL'];
        $this->templatetags['qq'] = $this->option['CFG_BLOG_QQ'];
        $this->templatetags['language'] = $this->option['CFG_BLOG_LANGUAGE'];
        $this->templatetags['keywords'] = $this->option['CFG_BLOG_KEYWORDS'];
        $this->templatetags['description'] = $this->option['CFG_BLOG_DESCRIPTION'];
        $this->templatetags['tqauthor'] = $this->option['CFG_BLOG_AUTHOR'];
        $this->templatetags['copyright'] = $this->option['CFG_BLOG_COPYRIGHT'];
        $this->templatetags['tqblogphp'] = $this->option['CFG_BLOG_PRODUCT_FULL'];
        $this->templatetags['tqblogphphtml'] = $this->option['CFG_BLOG_PRODUCT_FULLHTML'];
        ...
        $this->templatetags['captchaurl'] = &$this->captchaurl;

        $s = array(
            $option['CFG_SIDEBAR_ORDER'],
            $option['CFG_SIDEBAR2_ORDER'],
            $option['CFG_SIDEBAR3_ORDER'],
            $option['CFG_SIDEBAR4_ORDER'],
            $option['CFG_SIDEBAR5_ORDER']
        );
        foreach ($s as $k => $v) {
            $a = explode('|', $v);
            $ms = array();
        ...
        $this->template->SetPath($this->contentdir . 'theme/' . $this->theme . '/compile/');
        $this->template->tags = $this->templatetags;

        foreach ($GLOBALS['Filter_Plugin_Tqb_MakeTemplatetags'] as $fpname => &$fpsignal) {
    ...
    public function LoadTemplates() {
        ...
        //加载模板
        $dir = $this->contentdir . 'theme/' . $this->theme . '/template/';
        $files = GetFilesInDir($dir, 'php');
        foreach ($files as $sortname => $fullname) {
    ...
    public function BuildTemplate() {
        ...
        if (strpos($this->templates['commentpost'], 'inpVerify') === false) {
            $verify = '{if $option[\'CFG_COMMENT_VERIFY_ENABLE\'] && !$user.ID}<p><input type="text" name="inpVerify" id="inpVerify" class="text" value="" size="28" tabindex="4" /> <label for="inpVerify">' . $this->lang['msg']['captcha'] . '(*)</label><img style="width:{$option[\'CFG_VERIFYCODE_WIDTH\']}px;height:{$option[\'CFG_VERIFYCODE_HEIGHT\']}px;cursor:pointer;" src="{$article.CaptchaUrl}" alt="" title="" onclick="javascript:this.src=\'{$article.CaptchaUrl}&amp;tm=\'+Math.random();"/></p>{/if}';
        ...
        $dir = $this->contentdir . 'theme/' . $this->theme . '/compile/';
        ...
        //模板接口
        foreach ($GLOBALS['Filter_Plugin_Tqb_BuildTemplate'] as $fpname => &$fpsignal) {
...
#杂项
    ...
    function CheckPlugin($name) {
        $s = $this->option['CFG_BLOG_THEME'] . '|' . $this->option['CFG_USING_PLUGIN_LIST'];
    ...
    function ShowError($idortext, $file = null, $line = null) {
        ...
        foreach ($GLOBALS['Filter_Plugin_Tqb_ShowError'] as $fpname => &$fpsignal) {
    ...
    function ShowCaptcha($id = '') {
        ...
        foreach ($GLOBALS['Filter_Plugin_Tqb_ShowCaptcha'] as $fpname => &$fpsignal) {
            return $fpname($id); //*
        }

        $_vc = new Captcha();
        $_vc->GetImg();
        setcookie('tqbcaptcha' . md5($this->guid . $id), md5($this->guid . date("Ymd") . $_vc->GetCode()), null, $this->cookiespath);
    }

    function CheckCaptcha($vaidcode, $id = '') {

        foreach ($GLOBALS['Filter_Plugin_Tqb_CheckCaptcha'] as $fpname => &$fpsignal) {
            return $fpname($vaidcode, $id); //*
        }

        $original = GetVars('tqbcaptcha' . md5($this->guid . $id), 'COOKIE');
    ...
    function StartGzip() {
        if ($this->isgzip && isset($this->option['CFG_GZIP_ENABLE']) && $this->option['CFG_GZIP_ENABLE'])
