...
if($ispage){
  ...
  if(!$tqb->CheckRights('PageAll')&&$article->AuthorID!=$tqb->user->ID){$tqb->ShowError(6,__FILE__,__LINE__);die();}
}else{
  $blogtitle=$lang['msg']['article_edit'];
  if(!$tqb->CheckRights('ArticleAll')&&$article->AuthorID!=$tqb->user->ID){$tqb->ShowError(6,__FILE__,__LINE__);die();}
}
...
<form id="edit" name="edit" method="post" action="#">
  <div id="divEditLeft">
    ...
    <div id="divContent" style="clear:both;">
		<div id='cheader' class='editmod'><label for="editor_content" class="editinputname" ><?php echo $lang['msg']['content']?></label>&nbsp;&nbsp;<span id="timemsg"></span><span id="msg2"></span><span id="msg"></span><span class="editinputname" ></span><script type="text/javascript" src="../admin.php?act=misc&amp;type=autosave"></script></div>
...
<?php echo CreateOptoinsOfPostStatus($article->Status);?>
          ...
          <!-- origin -->
          <div id='origin' class='editmod'> <label for="cmbPostOrigin" class="editinputname" >来源</label>
            <select class="edit" style="width:180px;" size="1" name="Origin" id="cmbPostOrigin" onChange="edtOrigin.value=this.options[this.selectedIndex].value">
<?php echo CreateOptoinsOfPostOrigin($article->Origin);?>
            </select>
          </div>
          <!-- )origin -->
...
<?php echo CreateOptoinsOfTemplate($article->Template);?>
          ...
          <!-- Istop( --><?php if(!$ispage&&$tqb->CheckRights('ArticleAll')){?>
          ...
          <input type="text" name="AddNavbar" id="edtAddNavbar" value="<?php echo (int)$tqb->CheckItemToNavbar('page',$article->ID)?>" class="checkbox" />
          </div><?php }?>
          <!-- )Navbar -->
...
if($ispage){
  ...
  echo '<script type="text/javascript">AddHeaderIcon("'. $tqb->host . 'admin/image/admin/new_32.png' . '");</script>';
?>
...
window.onbeforeunload = function(){
  if (!isSubmit && (editor_api.editor.content.get()!=sContent)) return "<?php echo $tqb->lang['error'][71];?>";
}

function checkArticleInfo(){
  if(isSubmit)return false;
  document.getElementById("edit").action="<?php echo $ispage?'../admin.php?act=PagePst':'../admin.php?act=ArticlePst'?>";

  if(!editor_api.editor.content.get()){
    alert('<?php echo $tqb->lang['error'][70];?>');
    return false;
  }
  isSubmit=true;
}
  ...
  if(s.indexOf("<hr class=\"more\" />")>-1){
    editor_api.editor.intro.put(s.split("<hr class=\"more\" />")[0]);
      ...
      editor_api.editor.intro.put(s.split("<hr class=\"more\"/>")[0]);
	  	...
	  	editor_api.editor.intro.put(s.substring(0,<?php echo $tqb->option['CFG_ARTICLE_EXCERPT_MAX'];?>));
  	}
  }
	$("#divIntro").show();
