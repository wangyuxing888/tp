...
if(isset($_GET['source'])){
	if(GetVars('source','GET')=='theme'){
		...
		$mod->Source='theme';
		if($mod->FileName){
			$mod->Content=file_get_contents($tqb->contentdir . 'theme/' . $tqb->theme . '/include/' . $mod->FileName . '.php');
		}
	}
}elseif(isset($_GET['filename'])){

	$array=$tqb->GetModuleList(
		array('*'),
...
<label><input name="catalog_style" type="radio" value="0" <?php echo $tqb->option['CFG_MODULE_CATALOG_STYLE']=='0'?'checked="checked"':'';?> />&nbsp;<?php echo $lang['msg']['catalog_style_normal']?> </label>&nbsp;&nbsp;
<label><input name="catalog_style" type="radio" value="1" <?php echo $tqb->option['CFG_MODULE_CATALOG_STYLE']=='1'?'checked="checked"':'';?> />&nbsp;<?php echo $lang['msg']['catalog_style_tree']?> </label>&nbsp;&nbsp;
<label><input name="catalog_style" type="radio" value="2" <?php echo $tqb->option['CFG_MODULE_CATALOG_STYLE']=='2'?'checked="checked"':'';?> />&nbsp;<?php echo $lang['msg']['catalog_style_ul']?> </label>&nbsp;&nbsp;
...
function checkInfo(){
  document.getElementById("edit").action="../admin.php?act=ModulePst";
  ...
  if(!$("#edtHtmlID").val()){
	...
	<script type="text/javascript">ActiveLeftMenu("aModuleAdmin");</script>
	<script type="text/javascript">AddHeaderIcon("<?php echo $tqb->host . 'admin/image/admin/module_32.png';?>");</script>
  </div>
</div>
