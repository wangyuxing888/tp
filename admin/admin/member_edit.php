...
if(!$tqb->CheckRights('MemberAll')){
	if((int)$memberid<>(int)$tqb->user->ID) {$tqb->ShowError(6,__FILE__,__LINE__);}
}

$member=$tqb->GetMemberByID($memberid);
...
<?php if($tqb->CheckRights('MemberAll') && $tqb->user->ID<>$member->ID){?>
...
<?php }?>
	  ...
	  <p>
		...
		<input id="edtName" class="edit" size="40" name="Name" maxlength="20" type="text" value="<?php echo $member->Name;?>" <?php if(!$tqb->CheckRights('MemberAll'))echo 'readonly="readonly"';?> />
	  </p>
	  <p>
      ...
      <p>
		<span class="title"><?php echo $lang['msg']['qq']?>:</span><br />
		<input id="edtQQ" class="edit" size="40" name="QQ" type="text" value="<?php echo $member->QQ;?>" />
	  </p>
	  <p>
...
function checkInfo(){
  document.getElementById("edit").action="../admin.php?act=MemberPst<?php echo '&token='. $tqb->GetToken();?>";
  ...
  if($("#edtPassword").val()!==$("#edtPasswordRe").val()){
	...
	<script type="text/javascript">ActiveLeftMenu("aMemberAdmin");</script>
	<script type="text/javascript">AddHeaderIcon("<?php echo $tqb->host . 'admin/image/admin/user_32.png';?>");</script>
  </div>
</div>
