 ...
 * $Id: index.php 33828 2008-02-22 09:25:26Z team $
 */
...
if(isset($_GET['rsd'])){
	...
	echo '  <service>'."\n";
	echo '    <engineName>TQBlog</engineName>'."\n";
	echo '    <engineLink>http://www.tqtqtq.com/</engineLink>'."\n";
	echo '    <homePageLink>'.$tqb->host.'</homePageLink>'."\n";
	echo '    <apis>'."\n";
	echo '      <api name="WordPress" blogID="1" preferred="true" apiLink="'.$tqb->host.'admin/xmlrpc/" />'."\n";
	echo '      <api name="Movable Type" blogID="1" preferred="false" apiLink="'.$tqb->host.'admin/xmlrpc/" />'."\n";
	echo '      <api name="MetaWeblog" blogID="1" preferred="false" apiLink="'.$tqb->host.'admin/xmlrpc/" />'."\n";
	echo '      <api name="Blogger" blogID="1" preferred="false" apiLink="'.$tqb->host.'admin/xmlrpc/" />'."\n";
	echo '    </apis>'."\n";
	echo '  </service>'."\n";
	echo '</rsd>'."\n";
...
function tqb_getUsersBlogs(){
	...
	$strXML=str_replace("$%#1#%$",htmlspecialchars($tqb->host),$strXML);
	$strXML=str_replace("$%#2#%$",htmlspecialchars($tqb->guid),$strXML);
	$strXML=str_replace("$%#3#%$",htmlspecialchars($tqb->name),$strXML);

	echo $strXML;
}

function tqb_getCategories(){
	global $tqb;
...
<member><name>wp_slug</name><value><string>$%#10#%$</string></value></member>
	...
	$article->LoadInfoByID($id);
		if(($article->AuthorID!=$tqb->user->ID )&&(!$tqb->CheckRights('PageAll'))){$tqb->ShowError(11,__FILE__,__LINE_);}
...
function tqb_delPage($id){
	...
	if(DelPage()==true){
		...
		$tqb->ShowError(0,__FILE__,__LINE_);
	}

}

function tqb_deletePost($id){
...
<member><name>page_status</name><value><string>$%#15#%$</string></value></member>
	...
	$article= new Post;
	$article->LoadInfoByID($id);
	if(($article->AuthorID!=$tqb->user->ID )&&(!$tqb->CheckRights('ArticleAll'))){$tqb->ShowError(11,__FILE__,__LINE_);}
...
function tqb_getPostCategories($id){
	global $tqb;
...
function tqb_editPost($id,$xmlstring,$publish){
	global $tqb;
	...
	if($xml){
		...
		if(isset($post['dateCreated'])){
			...
			date_default_timezone_set($tqb->option['CFG_TIME_ZONE_NAME']);
		...
		if(isset($post['categories'])){
			...
			$_POST['CateID']=$tqb->GetCategoryByName($catename)->ID;
		}
...
function tqb_setPostCategories(){
	$strXML='<methodResponse><params><param><value><boolean>$%#1#%$</boolean></value></param></params></methodResponse>';
	$strXML=str_replace("$%#1#%$",1,$strXML);
	echo $strXML;
}
...
function tqb_editPage($id,$xmlstring,$publish){
	global $tqb;
	...
	if($xml){
		...
		if($tqb->CheckItemToNavbar('page',$id))$_POST['AddNavbar']=1;
...
function tqb_newMediaObject($xmlstring){
	global $tqb;
...
if($xml){
	...
	switch ($method) {
		case 'blogger.getUsersBlogs':
			...
			if(!$tqb->Verify_Original($username,$password)){ShowError(8,__FILE__,__LINE__);}
		...
		case 'mt.getPostCategories':
			...
			if(!$tqb->Verify_Original($username,$password)){ShowError(8,__FILE__,__LINE__);}
			if($tqb->CheckRights('ArticleEdt')){
				tqb_getPostCategories((integer)$xml->params->param[0]->value->string);
			}else{
				$tqb->ShowError(6,__FILE__,__LINE__);
			}
			break;
		...
		case 'metaWeblog.getRecentPosts':
			...
			if(!$tqb->Verify_Original($username,$password)){ShowError(8,__FILE__,__LINE__);}
			if($tqb->CheckRights('ArticleEdt')){
				tqb_getRecentPosts((integer)$xml->params->param[3]->value->int);
			}else{
				$tqb->ShowError(6,__FILE__,__LINE__);
			}
			break;
		case 'metaWeblog.getPost':
			...
			if(!$tqb->Verify_Original($username,$password)){ShowError(8,__FILE__,__LINE__);}
			if($tqb->CheckRights('ArticleEdt')){
				tqb_getPost((integer)$xml->params->param[0]->value->string);
			}else{
				$tqb->ShowError(6,__FILE__,__LINE__);
			}
			break;
		case 'blogger.deletePost':
			...
			if(!$tqb->Verify_Original($username,$password)){ShowError(8,__FILE__,__LINE__);}
			if($tqb->CheckRights('ArticleDel')){
				tqb_deletePost((integer)$xml->params->param[1]->value->string);
			}else{
				$tqb->ShowError(6,__FILE__,__LINE__);
			}
			break;
		case 'metaWeblog.editPost':
			...
			$password=(string)$xml->params->param[2]->value->string;
			if(!$tqb->Verify_Original($username,$password)){ShowError(8,__FILE__,__LINE__);}
			if($tqb->CheckRights('ArticlePst')){
				tqb_editPost((integer)$xml->params->param[0]->value->string,
		...
		case 'wp.editPage':
			...
			if(!$tqb->Verify_Original($username,$password)){ShowError(8,__FILE__,__LINE__);}
			if($tqb->CheckRights('PagePst')){
				tqb_editPage((integer)$xml->params->param[1]->value->string,
		...
		case 'wp.getPages':
			...
			$password=(string)$xml->params->param[2]->value->string;
			if(!$tqb->Verify_Original($username,$password)){ShowError(8,__FILE__,__LINE__);}
			if($tqb->CheckRights('PageEdt')){
				tqb_getPages((integer)$xml->params->param[3]->value->int);
			}else{
				$tqb->ShowError(6,__FILE__,__LINE__);
			}
			break;
		case 'wp.getPage':
			...
			if(!$tqb->Verify_Original($username,$password)){ShowError(8,__FILE__,__LINE__);}
			if($tqb->CheckRights('PageEdt')){
				tqb_getPage((integer)$xml->params->param[1]->value->string);
			}else{
				$tqb->ShowError(6,__FILE__,__LINE__);
			}
			break;
		case 'wp.deletePage':
			...
			$password=(string)$xml->params->param[2]->value->string;
			if(!$tqb->Verify_Original($username,$password)){ShowError(8,__FILE__,__LINE__);}
			if($tqb->CheckRights('PageDel')){
				tqb_delPage((integer)$xml->params->param[3]->value->string);
			}else{
				$tqb->ShowError(6,__FILE__,__LINE__);
			}
			break;
		case 'metaWeblog.newMediaObject':
			...
			if($tqb->CheckRights('UploadPst')){
				tqb_newMediaObject($xml->params->param[3]->value->struct->asXML());
			}else{
				$tqb->ShowError(6,__FILE__,__LINE__);
			}
			break;
		default:
			//logs($xmlstring);
			$tqb->ShowError(1,__FILE__,__LINE_);
			break;
	}
}
