...
function ViewList($page, $cate, $auth, $date, $tags, $isrewrite = false) {
    ...
    $articles = $tqb->GetArticleList(
        '*', $w, array('log_PostTime' => 'DESC'), array(($pagebar->PageNow - 1) * $pagebar->PageCount, $pagebar->PageCount), array('pagebar' => $pagebar), true
    );
...
function ViewPost($id, $alias, $isrewrite = false) {
    ...
    $comments = $tqb->GetCommentList(
        ...
        array('=', 'comm_LogID', $article->ID)
            ), array('comm_ID' => ($tqb->option['CFG_COMMENT_REVERSE_ORDER'] ? 'DESC' : 'ASC')), array(($pagebar->PageNow - 1) * $pagebar->PageCount, $pagebar->PageCount), array('pagebar' => $pagebar)
    );
    $rootid = array();
    ...
    $comments2 = $tqb->GetCommentList(
        ...
        array('=', 'comm_IsChecking', 0),
        array('=', 'comm_LogID', $article->ID)
            ), array('comm_ID' => ($tqb->option['CFG_COMMENT_REVERSE_ORDER'] ? 'DESC' : 'ASC')), null, null
    );
...
function PostComment() {
    ...
    foreach ($tqb->datainfo['Comment'] as $key => $value) {
        ...
        if ($key == 'IsChecking')
            $cmt->$key = ($tqb->option['CFG_COMMENT_CHECK']) ? 1 : 0;
        if (isset($_POST[$key])) {
...
function BuildModule_calendar($date = '') {
    ...
    $sql = $tqb->db->sql->Select(
            $tqb->table['Post'], array('log_ID', 'log_PostTime'), array(
        array('=', 'log_Type', '0'),
        array('=', 'log_Status', '0'),
        array('BETWEEN', 'log_PostTime', $fdate, $ldate)
            ), array('log_PostTime' => 'ASC'), null, null
    );
