...
<?php if ($user->ID>0) { ?>
    ...
    <input type="hidden" name="inpQQ" id="inpQQ" value="<?php  echo $user->QQ;  ?>" />
...
<?php }else{  ?>
    ...
    <p><input type="text" name="inpQQ" id="inpQQ" class="text" value="<?php  echo $user->QQ;  ?>" size="28" tabindex="2" /> <label for="inpQQ">QQ</label></p>
	<p><input type="text" name="inpEmail" id="inpEmail" class="text" value="<?php  echo $user->Email;  ?>" size="28" tabindex="3" /> <label for="inpEmail">邮箱</label></p>
	<p><input type="text" name="inpHomePage" id="inpHomePage" class="text" value="<?php  echo $user->HomePage;  ?>" size="28" tabindex="4" /> <label for="inpHomePage">网址</label></p>
<?php if ($option['CFG_COMMENT_VERIFY_ENABLE']) { ?>
	<p><input type="text" name="inpVerify" id="inpVerify" class="text" value="" size="28" tabindex="5" /> <label for="inpVerify">验证码(*)</label>
	<img style="width:<?php  echo $option['CFG_VERIFYCODE_WIDTH'];  ?>px;height:<?php  echo $option['CFG_VERIFYCODE_HEIGHT'];  ?>px;cursor:pointer;" src="<?php  echo $article->CaptchaUrl;  ?>" alt="" title="" onclick="javascript:this.src='<?php  echo $article->CaptchaUrl;  ?>&amp;tm='+Math.random();"/>
	</p>
<?php } ?>
